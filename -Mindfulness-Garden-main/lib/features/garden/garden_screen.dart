...
class _GardenScreenState extends State<GardenScreen>
  ...
  Offset _iso(double col, double row, Size size) {
    ...
    return Offset(centerX + wx * _zoom + _camX, centerY + wy * _zoom + _camY);
  }
  ...
  void initState() {
    ...
    // Initialize auto-season based on current date
    _initAutoSeason();
  ...
  void _initGrid() {
    ...
    for (final s in starters) {
      ...
      if (t.tileType == TileType.soil) {
        ...
        t.xpValue = 10 + s.$4 * 5;
        t.plantedAt = DateTime.now().subtract(
          Duration(hours: _rng.nextInt(24)),
        );
      }
    }
  }
  ...
  /// Initialize season based on current date automatically
  void _initAutoSeason() {
    try {
      final autoTheme = AutoThemeService();
      final autoSeason = autoTheme.getCurrentGardenSeason();

      // Map GardenSeason enum to Season enum
      switch (autoSeason) {
      ...
      // Optionally, users can still manually change seasons
    } catch (e) {
      // Fallback to spring if auto-season fails
      _season = Season.spring;
    }
  }
      ...
      try {
        ...
        for (int i = 0; i < list.length && i < _achievements.length; i++) {
          ...
          if (list[i]['unlockedAt'] != null) {
            _achievements[i].unlockedAt = DateTime.tryParse(
              list[i]['unlockedAt'],
            );
          }
        }
  ...
  Future<void> _saveData() async {
    ...
    await LocalStorageService.saveSetting(
      'garden_total_harvests',
      _totalHarvests,
    );
    await LocalStorageService.saveSetting(
      'garden_total_planted',
      _totalPlanted,
    );
    await LocalStorageService.saveSetting(
      'garden_total_watered',
      _totalWatered,
    );
    await LocalStorageService.saveSetting(
      'garden_seasons_visited',
  ...
  void _unlockAchievement(AchievementType type, bool condition) {
    ...
    if (!ach.unlocked && condition) {
      ...
      _pushNotification(
        ...
        '${ach.emoji} ${ach.title} — +${ach.reward} 🪙',
        Colors.amber,
      );
      HapticFeedback.heavyImpact();
    }
  ...
  void _pushNotification(String title, String body, Color color) {
    _notifications.insert(
      0,
      _Notification(title: title, body: body, color: color),
    );
  ...
  void _addXP(int amount, double col, double row, String text) {
    ...
    if (_xp >= _xpToNext) {
      ...
      _pushNotification(
        '⭐ Level Up!',
        'Garden Level $_gardenLevel! New plants unlocked!',
        Colors.yellow,
      );
      HapticFeedback.heavyImpact();
    }
  ...
  void _doPlant(GardenTile t, Offset tap, Size size) {
    ...
    setState(() {
      ...
      if (t.isRare) {
        ...
        _pushNotification(
          '✨ Rare Plant!',
          'You planted a rare ${data['name']}! +2x rewards!',
          Colors.purple,
        );
      } else {
  ...
  void _doHarvest(GardenTile t, Offset tap, Size size) {
    ...
    setState(() {
      ...
      _addXP(
        t.xpValue * t.growthStage * (t.isRare ? 2 : 1),
        t.col.toDouble(),
        t.row.toDouble(),
        '+${t.xpValue * t.growthStage} XP 🌟',
      );
      _labels.add(
        FloatingLabel(
          x: tap.dx / size.width,
  ...
  void _buyShopItem(ShopItem item) {
    ...
    setState(() {
      ...
      if (e.containsKey('rare_seed')) {
        ...
        _pushNotification(
          '✨ Rare Seed!',
          'Next plant you grow will be rare!',
          Colors.purple,
        );
      }
      if (e.containsKey('grow_boost')) {
        ...
        _pushNotification(
          '⚡ Growth Boost!',
          '2x growth speed for 60 seconds!',
          Colors.yellow,
        );
      }
    });
          ...
          if (_cfg.hasFireflies)
            AnimatedBuilder(
              ...
              builder: (_, __) => Positioned.fill(
                child: IgnorePointer(
                  child: CustomPaint(
                    painter: _FireflyPainter(_fireflies, size, _glowAnim.value),
                  ),
                ),
              ),
            ),
              ...
              (n) => AnimatedOpacity(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [
                      BoxShadow(color: n.color.withOpacity(0.2), blurRadius: 8),
                    ],
                  ),
                  child: Row(
                    children: [
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Text(
              '2x Growth: ${_growBoostSecondsLeft}s',
              style: const TextStyle(
                color: Colors.yellow,
                fontSize: 11,
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                _weatherBadge(
                  'Growth',
                  '${(_cfg.growthMult * 100).toInt()}%',
                  _cfg.growthMult >= 1.0 ? Colors.green : Colors.orange,
                ),
                _weatherBadge(
                  'Wind',
                  _cfg.windStrength > 0.5 ? 'Strong' : 'Calm',
                ...
                if (_cfg.hasSnow)
                  _weatherBadge('Snow', 'Falling', Colors.white),
                if (_cfg.hasFireflies)
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            ...
                            style: TextStyle(
                              color:
                                  canAfford ? Colors.greenAccent : Colors.red,
                              fontSize: 7,
                              fontWeight: FontWeight.w600,
                            ),
                          ),
  ...
  Widget _buildTilePanel(GardenTile t) {
    ...
    final isSeasonalBonus = _cfg.seasonalPlants.contains(
      t.plantType?.name ?? '',
    );
    return Container(
      padding: const EdgeInsets.all(14),
          ...
          Row(
            children: [
              Stack(
                children: [
                  Text(
                    _plantEmoji(t.plantType!, t.growthStage),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Text(
                          ...
                          style: TextStyle(
                            color:
                                t.isRare ? Colors.purple : Colors.greenAccent,
                            fontWeight: FontWeight.w800,
                            fontSize: 13,
                          ),
                        ),
                    ...
                    Text(
                      'Stage ${t.growthStage}/5  •  (${t.col},${t.row})',
                      style: const TextStyle(
                ...
                child: Row(
                  children: [
                    Text(
                      ...
                      style: TextStyle(
                        fontSize: 24,
                        color: a.unlocked ? null : const Color(0x44FFFFFF),
                      ),
                    ),
                    const SizedBox(width: 10),
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            a.title,
                            style: TextStyle(
                              color: a.unlocked ? Colors.amber : Colors.white38,
                              fontWeight: FontWeight.w700,
                              fontSize: 13,
                            ),
                          ),
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          ...
                          style: TextStyle(
                            color:
                                q.completed ? Colors.greenAccent : Colors.white,
                            fontWeight: FontWeight.w700,
                            fontSize: 13,
                          ),
                        ),
                      ...
                      Expanded(
                        child: ClipRRect(
                          ...
                          child: LinearProgressIndicator(
                            value: q.fraction,
                            backgroundColor: Colors.white12,
                            color: q.completed ? Colors.green : Colors.amber,
                            minHeight: 6,
                          ),
                        ),
                      ),
...
class _GardenPainter extends CustomPainter {
  ...
  Offset _iso(double c, double r) {
    ...
    return Offset(centerX + wx * zoom + camX, centerY + wy * zoom + camY);
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (cfg.hasButterflies) {
      for (final b in butterflies) {
        _drawCreature(
          canvas,
          b.x * screenSize.width,
          b.y * screenSize.height,
          b.emoji,
          14,
        );
      }
  ...
  void _drawTile(Canvas canvas, GardenTile t, double c, double r) {
    ...
    canvas.drawPath(
      leftPath,
      Paint()..color = cfg.groundDark.withOpacity(0.55),
    );
    ...
    canvas.drawPath(
      rightPath,
      Paint()..color = cfg.groundDark.withOpacity(0.38),
    );
  ...
  void _drawPlant(Canvas canvas, GardenTile t, Offset center) {
    ...
    tp.paint(
      canvas,
      Offset(center.dx - tp.width / 2, center.dy - tp.height * 0.88),
    );
    ...
    if (t.health < 0.75) {
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTWH(bx, by, bw * t.health, 4),
          const Radius.circular(2),
        ),
    ...
    if (t.water < 0.25) {
      _drawEmoji(
        canvas,
        Offset(center.dx + fs * 0.35, center.dy - fs * 0.9),
        '🥵',
        10,
      );
    }
  ...
  void _drawMeditator(Canvas canvas, Meditator m) {
    ...
    final tp = TextPainter(
      text: TextSpan(
        text: m.emoji,
        style: TextStyle(fontSize: 22 * breathScale),
      ),
      textDirection: TextDirection.ltr,
    )..layout();
    ...
    if (m.speechBubble != null && m.speechLife > 0) {
      ...
      canvas.drawRRect(
        bubbleRect,
        Paint()..color = Colors.white.withOpacity(alpha * 0.95),
      );
      // Tail triangle
      final tailPath = Path()
      ...
      st.paint(
        canvas,
        Offset(center.dx - st.width / 2, bubbleCy - st.height / 2),
      );
    }
    ...
    if (season == Season.monsoon) {
      _drawEmoji(
        canvas,
        Offset(center.dx, center.dy - tp.height - 14),
        '☂️',
        14,
      );
    }
    if (season == Season.winter || season == Season.snowfall) {
      _drawEmoji(
        canvas,
        Offset(center.dx + 8, center.dy - tp.height + 4),
        '🧣',
        10,
      );
    }
...
class _RainPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final d in drops) {
      ...
      canvas.drawLine(
        Offset(x, y),
        Offset(x + d.windX * ss.width * 8, y + len),
        paint,
      );
    }
  }
...
class _BurstPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    for (final p in particles) {
      canvas.drawCircle(
        Offset(p.x * ss.width, p.y * ss.height),
        p.size / 2,
